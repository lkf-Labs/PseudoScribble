...
We developed an interactive annotation and quantitative analysis system, where users can optionally enable the pseudo-scribble module and specify the number of scribbles automatically generated by the model. Through simple mouse-click interactions on the image, users provide lightweight feedback to guide the model in extracting meibomian glands. Based on the segmentation results, the system further enables automatic computation of various morphological parameters for each gland.


 <img src="./assets/img/demo.png" alt="drawing"/>

 Feel free to explore it using the following command:
