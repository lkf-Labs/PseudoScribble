...
def get_dataset(dataset_name, cfg):
    dataset = MeibomianGlandsDataset(cfg.MeibomianGlands_PATH,split='test')
    return dataset
...
def get_dice(gt_mask, pred_mask, ignore_label=-1):
    ...
    obj_gt_mask = gt_mask == 1
    intersection = np.logical_and(np.logical_and(pred_mask, obj_gt_mask), ignore_gt_mask_inv).sum()
    dice = 2 * intersection / (pred_mask.sum() + obj_gt_mask.sum())
...
def get_hd(gt_mask, pred_mask):
    # Convert the masks to uint8
    gt_mask = gt_mask.astype(np.uint8)
    pred_mask = pred_mask.astype(np.uint8)

    gt_tensor = torch.tensor(gt_mask, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    pred_tensor = torch.tensor(pred_mask, dtype=torch.float32).unsqueeze(0).unsqueeze(0)

    # HausdorffDistanceMetric
    hd_metric = monai.metrics.HausdorffDistanceMetric(include_background=False)

    # Hausdorff Distance
    hd_value = hd_metric(pred_tensor, gt_tensor)
