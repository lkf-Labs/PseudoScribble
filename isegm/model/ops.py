...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, batchsize, rows, cols):
        if self.cpu_mode:
            ...
            # record the x and y coordinates of each pixel
            ...
            # record the x and y axis distances from each pixel to the center of each circle
            coords.add_(-add_xy)
            if not self.use_disks:
                coords.div_(self.norm_radius * self.spatial_scale)

            coords.mul_(coords)
            # obtain the squared distance from each pixel to the circle center
            coords[:, 0] += coords[:, 1]
            coords = coords[:, :1]
            ...
            # obtain the squared distance from each pixel to the nearest circle center
            coords = coords.min(dim=1)[0]  # -> (bs * num_masks * 2) x 1 x h x w
            # batch × 2 × h × w
            coords = coords.view(-1, 2, rows, cols)

        if self.use_disks:
            # set the square of the distance less than the specified radius (default is 5) to 1, and the rest to 0
    ...
    def forward(self, x, coords):  # batch h w
...
class CurvesMap(nn.Module):
    """
        scribbles encoing
    """
    def __init__(self, ):
        super(CurvesMap, self).__init__()

    def forward(self, img, points):
        num_points = points.shape[1] // 2
        batchsize, h, w = img.shape[0], img.shape[2], img.shape[3]

        # initialize pos_map and neg_map
        pos_map = torch.zeros((batchsize, 1, h, w), device=points.device)
        neg_map = torch.zeros((batchsize, 1, h, w), device=points.device)

        pos_points = points[:, :num_points, ...]
        neg_points = points[:, num_points:, ...]

        for i in range(batchsize):
            pos_batch_points = pos_points[i]
            neg_batch_points = neg_points[i]

            # filter out the coordinates that meet the conditions
            pos_x = pos_batch_points[:, 0]
            pos_y = pos_batch_points[:, 1]
            pos_mask = (pos_x > 0.) & (pos_y > 0.)

            neg_x = neg_batch_points[:, 0]
            neg_y = neg_batch_points[:, 1]
            neg_mask = (neg_x > 0.) & (neg_y > 0.)

            # filter the points that meet the conditions
            pos_x = pos_x[pos_mask].long()
            pos_y = pos_y[pos_mask].long()

            neg_x = neg_x[neg_mask].long()
            neg_y = neg_y[neg_mask].long()

            # use scatter_ to update pos_map and neg_map
            pos_map[i, 0, pos_x, pos_y] = 1.
            neg_map[i, 0, neg_x, neg_y] = 1.
