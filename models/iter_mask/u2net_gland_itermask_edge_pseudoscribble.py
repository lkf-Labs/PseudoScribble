...
from isegm.model.is_u2net_pseudoscribble_edge_model import U2netEdgePseudoScribbleModel
from isegm.utils.exp_imports.default import *

MODEL_NAME = 'gland_pseudoscribble'
...
def init_model(cfg):
    ...
    model = U2netEdgePseudoScribbleModel(use_leaky_relu=True,use_rgb_conv=False, use_disks=True, norm_radius=5, with_prev_mask=True,is_unet=True)
...
def train(model, cfg, model_cfg):
    ...
    loss_cfg.instance_aux_loss = MutiNFLLoss()  # NFL for deep supervision
    loss_cfg.instance_aux_loss_weight = 1.0
    # fpmap loss
    # loss_cfg.fp_aux_loss = MutiNFLLoss(alpha=0.75, gamma=2)
    # loss_cfg.fp_aux_loss = MutilWeightedCrossEntropyLoss()
    loss_cfg.fp_aux_loss = MutiSoftIoULoss()  #  IoU loss deep supervision
    loss_cfg.fp_aux_loss_weight = 0.0
    # fnmap loss
    # loss_cfg.fn_aux_loss = MutiNFLLoss(alpha=0.75, gamma=2)
